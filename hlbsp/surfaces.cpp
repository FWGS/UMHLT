...
void            SubdivideFace(face_t* f, face_t** prevptr)
{
    ...
    // compute subdiv size and texture step
    max_surface_extent = MAX_SURFACE_EXTENT;
    subdivide_size = g_subdivide_size;
    texture_step = TEXTURE_STEP;

    // landscape with custom subdivision or lightmap resolution
    if( GetSurfaceExtent( tex ) != max_surface_extent || GetTextureStep( tex ) != texture_step )
    {
    ...
    for (axis = 0; axis < 2; axis++)
    {
        while (1)
        {
			...
			const int maxlightmapsize = subdivide_size / texture_step + 1;
...
#else
            plane.dist = (mins + subdivide_size - texture_step) / v; //plane.dist = (mins + subdivide_size - 16) / v; //--vluzacn
#endif
            next = f->next;
